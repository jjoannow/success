...
trigger InitiateDataDeploymentUponApproval on Deployment_Request__c (before update) {
    ...
    for (Deployment_Request__c updatedDeploymentRequest : Trigger.new) {
        if(updatedDeploymentRequest.Initiate_Deployment_After_Approval__c == TRUE && updatedDeploymentRequest.Deployment_Initiated__c != TRUE){  
            ...
            SimpleProdlyApexApi spaa = new SimpleProdlyApexApi(updatedDeploymentRequest);
        }
        else{
            // Do nothing here to avoid unecessary and runaway execution of the trigger and class 
        }
    }    
